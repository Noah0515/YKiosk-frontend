...
function App() {
  ...
  const checkMyInfo = async () => {
    try {
      ...
      const response = await axios.get("http://localhost:8080/YKiosk/api/user/me", {
        withCredentials: true 
      });
      setUserInfo(response.data);
      console.log("서버에서 받은 유저 정보:", response.data);
    } catch (error) {
    ...
    <div style={{ padding: '50px', textAlign: 'center', fontFamily: 'sans-serif' }}>
      ...
      {!userInfo ? (
        ...
        <div style={{ marginTop: '20px', border: '1px solid #ddd', padding: '20px' }}>
          <h3>🎉 로그인 성공!</h3>
          <p><strong>아이디(Hash):</strong> {userInfo.userId}</p>
          <p><strong>닉네임:</strong> {userInfo.name}</p>
